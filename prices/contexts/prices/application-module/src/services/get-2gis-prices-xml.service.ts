...
import { map2gisCategoriesName } from '../mappers/index.js'
...
export class Get2gisPricesXmlService {
  async process(queryData: QueryDataType): Promise<string> {
    ...
    const [mappedGoods, mappedGoodsCategories] = map2gisCategoriesName(
      goodsData2gis,
      goodsCategoryData2gis
    )

    const twoGisCategories = map2gisCategories(mappedGoodsCategories)
    const twoGisOffers = map2gisOffers(mappedGoods)
